...
parser = argparse.ArgumentParser(
    description="Generate training metrics plot from log file"
)
parser.add_argument(
    "--log_file",
    type=str,
...
fig.suptitle(
    f"MLX Distributed M3 Ultra + M2 Ultra - Phi-4 Training Metrics\nAvg Tokens/sec: {avg_tokens_per_sec:.2f} | Avg It/sec: {avg_iter_per_sec:.4f}",
    fontsize=14,
)
...
ax2.plot(
    iterations, learning_rates, "g-", marker="^", markersize=4, label="Learning Rate"
)
...
print(
    f"Data points extracted: {len(iterations)} train, {len(val_iterations)} validation"
)
