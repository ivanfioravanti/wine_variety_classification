...
def generate_chart(summary_df, timestamp):
    ...
    country = df_sorted['country'].iloc[0] if 'country' in df_sorted.columns else 'Unknown'
    ...
    plt.ylabel("Accuracy (%)")
    plt.xlabel("Model")
    plt.title(f"{country} Wines variety classification from reviews accuracy (Best to Worst)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run wine variety prediction tests across different LLM providers')
    parser.add_argument('--generate-chart', action='store_true', help='Generate chart from most recent results without running new tests')
    parser.add_argument('--no-provider-csv', action='store_true', help='Disable saving individual provider results to CSV files')
    parser.add_argument('--summary', help='Specific summary file name to generate chart from (e.g., summary_20250105_095642.csv)')
    ...
    if args.generate_chart:
        ...
        for module, models in module_models:
            ...
            try:
                ...
                # Save the detailed results for this module if not disabled
                if not args.no_provider_csv:
                    df.to_csv(f"results/{module.__name__}_{timestamp}.csv", index=False)
