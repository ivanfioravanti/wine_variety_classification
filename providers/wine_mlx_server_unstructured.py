...
DEFAULT_MODELS = ["mlx-community/Qwen3-1.7B-bf16"]
...
df_country_subset = df_country.sample(n=100, random_state=RANDOM_SEED)
...
def process_example(index, row, model, df, progress_bar):
    ...
    try:
        ...
        predicted = call_model(model, prompt)
        df.at[index, model + "-variety"] = predicted
        correct = predicted == row["variety"]
        icon = "✅" if correct else "❌"
        print(f"{icon} Predicted: {predicted}, Actual: {row['variety']}")
